...
$(function() {
    ...
    // 슬라이드 박스 가로크기 지정
    // 슬라이드 크기 : 각 슬라이드 가로크기 x 슬라이드 개수
    list.css( { width : totalSlideWidth + 50 })

    //  이전 이미지 세팅
    // 슬라이드를 절반으로 나눠서 앞으로 가져올 슬라이드 개수 4 5 1 2 3
    // 5 / 2 = 2.5, parseInt(2.5)  ----(정수만)--->  2
    let initNum = parseInt(slideCount / 2)

    for (let i = 0; i < initNum; i++) {
        // A.prependTo(B)         : A를 부모요소의 (B)의 가장 첫번째 자식요소로 추가한다.
    ...
    $('.dot').on('click', function()  {
        ...
        // 슬라이드 위에 마우스 올렸을때 자동재생 멈춤
        $('.slide-list, .btn-box, .dots-box').on('mouseover', function() {
            pause = true
        })

        // 슬라이드 위에 마우스에서 벗어났을때 자동재생 시작
